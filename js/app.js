...
function checkout() {
  ...
  // Si no hay sesión, igual dejamos continuar (muchas tiendas lo permiten)
  ...
  const whatsappMessage = 
    ...
    `Pedido #: ${orderId}%0A` +
    (buyerName !== 'Cliente' ? `Nombre: ${buyerName}%0A` : '') +
  ...
  // 2. Si hay link de Mercado Pago configurado, abrir también el pago
  if (MERCADOPAGO_LINK && MERCADOPAGO_LINK.trim() !== '') {
    // Pequeña espera para que primero se abra WhatsApp
    setTimeout(() => {
      window.open(MERCADOPAGO_LINK, '_blank');
    }, 800);
    showToast('Redirigiendo a Mercado Pago para pagar...', 'success');
  } else {
    // Aún no hay link → avisar
    showToast('Pedido enviado por WhatsApp. Falta configurar el link de Mercado Pago.', 'success');
  }
