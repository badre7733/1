...
// WhatsApp de Agale Colombia (ya configurado)
const WHATSAPP_NUMBER = "573337475898";

// ===== DATA =====
const DEFAULT_PRODUCTS = [
  {
    id: 1,
    title: "Audífonos Bluetooth TWS Pro",
    category: "tecnologia",
    price: 89900,
    oldPrice: 129900,
    stock: 50,
    desc: "Audífonos inalámbricos con cancelación de ruido, batería de 30 horas y micrófono integrado. Ideal para deporte y oficina.",
  ...
  {
    id: 2,
    title: "Camiseta Oversize Premium Unisex",
    category: "moda",
    price: 45900,
    oldPrice: 69900,
    stock: 120,
    desc: "Camiseta de algodón peinado 100%, corte oversize, disponible en varios colores. Envío desde Bogotá.",
  ...
  },
  {
    ...
    oldPrice: 99000,
    stock: 35,
    desc: "Control por app y voz (Alexa/Google). 16 millones de colores, temporizador y modos de ambiente.",
  ...
  {
    id: 4,
    title: "Kit Skincare Coreano Completo",
    category: "belleza",
    price: 119900,
    oldPrice: 159900,
    stock: 40,
    desc: "Set de 5 productos: limpiador, tónico, serum, crema y protector solar. Piel hidratada y luminosa.",
  ...
  },
  {
    ...
    oldPrice: 49900,
    stock: 80,
    desc: "5 niveles de resistencia, material látex natural, incluye bolsa de transporte. Ideal para gym en casa.",
  ...
  {
    id: 6,
    title: "Soporte Celular Magnético Auto",
    category: "accesorios",
    price: 28900,
    oldPrice: 39900,
    stock: 200,
    desc: "Base magnética fuerte para rejilla de ventilación. Compatible con todos los smartphones.",
    image: "https://images.unsplash.com/photo-1601784551446-20c9e07cdbdb?w=400&h=400&fit=crop",
    seller: "AutoAccesorios",
    sellerId: "sys",
    dropship: true,
    provider: "Dropi"
  },
  {
    id: 7,
    title: "Reloj Inteligente Smart Watch X3",
    category: "tecnologia",
    price: 149900,
    oldPrice: 199900,
    stock: 25,
    desc: "Pantalla AMOLED, monitor de ritmo cardíaco, SpO2, GPS, 100+ modos deportivos y 7 días de batería.",
  ...
  {
    id: 8,
    title: "Zapatillas Running Livianas",
    category: "deportes",
    price: 159900,
    oldPrice: 219900,
    stock: 45,
    desc: "Suela de alta amortiguación, transpirables y ligeras. Perfectas para correr o caminar diariamente.",
  ...
  {
    id: 9,
    title: "Organizador de Cocina Multiusos",
    category: "hogar",
    price: 39900,
    oldPrice: null,
    stock: 60,
    desc: "Estructura metálica resistente, ideal para utensilios, especias o baño. Fácil de armar.",
  ...
  },
  {
    ...
    oldPrice: 119900,
    stock: 55,
    desc: "Compartimento para laptop 15.6\", puerto USB externo, cremalleras ocultas y material impermeable.",
  ...
  {
    id: 11,
    title: "Crema Facial Ácido Hialurónico",
    category: "belleza",
    price: 54900,
    oldPrice: 79900,
    stock: 70,
    desc: "Hidratación profunda 24h, textura ligera, apta para todo tipo de piel. Resultados visibles en 7 días.",
    image: "https://images.unsplash.com/photo-1571781926291-c77df043d1ac?w=400&h=400&fit=crop",
    seller: "BeautyLab",
    sellerId: "sys",
    dropship: false
  },
  {
    id: 12,
    title: "Mouse Gamer RGB Inalámbrico",
    category: "tecnologia",
    price: 79900,
    oldPrice: 109900,
    stock: 40,
    desc: "Sensor 16000 DPI, 6 botones programables, batería recargable y RGB personalizable.",
...
function saveCurrentOrder(status) {
  const buyerName = currentUser ? currentUser.name : 'Cliente';
  const buyerEmail = currentUser ? currentUser.email : '';
  const total = cart.reduce((s, c) => s + c.price * c.qty, 0);
...
function buildWhatsAppMessage(orderId, total, buyerName, buyerEmail) {
  const itemsText = cart.map(c => `• ${c.qty}x ${c.title} — ${formatPrice(c.price * c.qty)}`).join('%0A');
  return (
    `Hola Agale Colombia 👋%0A%0A` +
    `Quiero confirmar este pedido:%0A%0A` +
    `${itemsText}%0A%0A` +
    `*Total: ${formatPrice(total)}*%0A` +
    `Pedido #: ${orderId}%0A` +
    (buyerName && buyerName !== 'Cliente' ? `Nombre: ${buyerName}%0A` : '') +
    (buyerEmail ? `Email: ${buyerEmail}%0A` : '') +
    `%0AGracias.`
  );
}

// Botón principal: Pagar con Mercado Pago (NO abre WhatsApp)
function checkout() {
  if (cart.length === 0) {
  ...
  saveCurrentOrder('Pendiente de pago');

  if (MERCADOPAGO_LINK && MERCADOPAGO_LINK.trim() !== '') {
    // Abrir Mercado Pago (funciona en PC y celular)
    window.location.href = MERCADOPAGO_LINK;
    // Limpiar carrito después de redirigir
    cart = [];
    saveCart();
    updateCartUI();
  } else {
...
// Botón opcional: Enviar pedido por WhatsApp (solo si el cliente quiere)
function sendOrderWhatsApp() {
  ...
  const { orderId, total, buyerName, buyerEmail } = saveCurrentOrder('Enviado por WhatsApp');
  const msg = buildWhatsAppMessage(orderId, total, buyerName, buyerEmail);
  ...
  updateCartUI();
  toggleCart();
  showToast('Pedido preparado para WhatsApp', 'success');
}
